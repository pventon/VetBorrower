...
export async function GetCorporationById(id) {
  return await CorporationsModel.findById(id);
}

export async function AddCorporation(data) {
  const record = new CorporationsModel(data);
  return await record.save();
}

export async function UpdateCorporation(id, data) {
  return await CorporationsModel.findByIdAndUpdate(id, data, { returnDocument: 'after' });
}
